 ...
 * - 배열의 위치(index)값은 0부터 시작함. 위치값은 정수임.
 *  score[0], score[1], score[2].....
 *  - 배열은 시작과 끝이 있음. => 배열의크기 => 배열의 요소 개수
 *   문법 에러는 아니지만, 실행 에러 => indexOFBound....
 *   -선언방법
 *   타입[] 배열변수명; <= 추천
 *   타입 배열변수명[];
 *    
 ...
 *     
 *  - String 배열
 *    String 클래스는 char 배열에 여러가지 편리한 기능을(메소드)을 추가한 타입임.
 *    String 클래스에 저장된 내용은 읽을 수만 있고 변경 할 수 없다.
 *    
 *  - 다차원 배열
 *    1차원 배열 : 행으로만 구성됨.
 *    2차원 배열 : 행, 열로 구성됨.
...
public class ArrayEx {
	...
	public static void main(String[] args) {
		...
		int[] iArr1 = new int[10]; // 각 요소를 초기화 하지 않은 상태. 
		...
		//원본, 복사본 모두 필요한 경우
		//복사본이 원본이 되어, 원본이 필요가 없게 되는경우. => 가비지 컬렉트 대상.
		arr2 = tmp; // arr2는 기존의 배열의 시작주소가 없어지고,
					// 대신에 복사본의 배열의 시작주소가 저장이 됨으로
					// arr2는 가비지 컬렉터 대상이 됨.
			...
			System.out.println(sum+ "," + average); // sum = 400, average = 80
		...
		System.out.println(tmpScore4); //[I@35f983a6  -> 주소값
		...
		// 빈도수 구하기
		// [7, 1, 7 ,6, 7] 데이터 배열
		// 카운트 배열에서 해당 데이터 배열의 요소값에 해당하는 위치에 1씩 증가.
		...
		System.out.println(Arrays.toString(counter2)); //배열은 자동 초기화된다.
		...
		System.out.println(Arrays.toString(name2)); //[kim, park, hong] 출력
		
		String[] name3 = new String[] {"kim","park", "hong"}; //[kim, park, hong] 출력
		...
		// 주소 비교해서 같으면 같은 변수임.
		if(str == str2) System.out.println("equal"); // 달라서 출력x kim ==kim2
		
		if(str.equals(str2)) System.out.println("문자열 동일"); //달라서 출력x kim != kim2
