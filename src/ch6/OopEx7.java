...
import ch6.Outer2.InstanceInner;
import ch6.Outer2.StaticInner;
import ch6.Outer4.Inner;
 ...
 *   - 인터페이스를 활용한 약결합 예제 => 리팩토링 책, 디자인 패턴  
 ...
 * - 기반 클래스 이름 또는 인터페이스의 이름을 사용해서 정의.
 * - 기반 클래스도 단일 상속, 인터페이스도 단일 implement 
 * 
 * 
 ...
 * 
 * 안드로이드 앱 사용자가 버튼을 버튼을 클릭했다는 것은 아래의 addActionListnener 가
 * 호출됨
 * b.addActionListener( 
...
class AnnoymousClass{
   ...
   void myMethod(){
      Object obj3 = new Object() { void method() {} }; // 익명 클래스
...
// ------------------- 멤버 메서드, 클래스 메소드에서 멤버 클래스 및 지역 클래스 접근 ---------------------
class Outer2 {
   class InstanceInner{}
   ...
   static void staticMethod(){
      // 특성 정리 : 스태틱은 스새틱에만 접근 가능. 스태틱은 인스턴스에 접근못함.
      // InstanceInner obj1 = new InstanceInner();
      StaticInner obj2  = new StaticInner();
      //LocalInner lv;
...
// ------------------- Outer 클래스의 인스턴스 및 스태틱 클래스의 멤버 변수 사용 ------------------------
class Outer3{
   class InstanceInner {
      int iv = 100;
   }
...
public class OopEx7 {
   ...
   public static void main(String[] args) {
      ...
      System.out.println(InstanceInner.CONST); // static 라서 가능함.
      
      // IntanceInner 는 인스턴스 클래스.
