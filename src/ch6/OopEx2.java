 ...
 *   다중 상속 : 기반 클래스가 여러 개. => C++
 *   
 *   JAVA 에서 멀티 상속처럼 사용하고 싶은 경우.
 ...
 *   오버라이딩 : 상속관계. 파생된 쪽에서 부모 메소드와 동일한 것을 수정 정의하는 것.
 *   
 * - super
 ...
 *   
 *   - 인스턴스 생성시 선택
 *   클래스 : 어떤 클래스의 인스턴스를 생성할 것인가? => 클래스 설게
...
class Point3D extends Point{ // 파생 클래스, 3차원
   int z;
   ...
   Point3D(int x, int y){
      ...
      // 따라서, Point2가 먼저 인스턴스로 생성되어야
      // Point3D 의 인스턴스를 생성할 수 있음.
      // 그래서, 파생 클래스의 생성자에서 반드시 첫 줄에 기반 클래스의
      // 생성자를 호출해서, 기반 클래스의 인스턴스를 먼저 생성하게 해야 함.
      super(x,y); // this : 자기참조, this() : 생성자 호출
      int a = 10;
   }
...
// JAVA에서의 다중 상속과 유사한 클래스 작성
// TV, VCR class를 이용해서 다중 상속처럼 사용하려면,
...
class TvVcr extends Tv2 {
   ...
   // TvVcr은 별도의 새로운 제품임으로 자기만의 기능을 구현함.
   void play() {
...
class PockerCard {
   ...
   // 카드별 무늬 및 숫자
   int kind;
   int number; //1. 무늬를 나타내는 정보,  2. 화면 출력용 데이터 배열의 인덱스 정보
   ...
   public String toString() {
      //1. 화면 출력용 데이터를 한군데서 관리
      //2. 코드 작성 시 가독성과 사용 편리성을 높이기 위함.
